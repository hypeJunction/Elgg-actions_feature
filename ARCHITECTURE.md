# actions_feature — Architecture (Elgg 4.x)

## Overview

`actions_feature` is a small Elgg 4.x plugin that lets admins mark entities
as "featured". It provides permission hooks, entity-menu actions, and a DB
...
| `ActionsFeature\Permissions` | `canFeature(entity, user?)` — checks allowed type + admin status via plugin hooks |
| `ActionsFeature\Menus` | `entityMenu(Hook)` — injects Feature/Unfeature items into `menu:entity` |
